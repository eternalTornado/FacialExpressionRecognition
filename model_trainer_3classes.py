...
num_classes = 7
num_classes_alternative = 3
batch_size = 125
epochs = 35

class_names = ["angry", "disgust", "fear", "happy", "sad", "surprise", "neutral"]
class_names_alternative = ["negative", "positive", "neutral"]
...
for i in range(num_of_instances):
	try:
		...
		if int(emotion) in [0,1,2,4]:
			emotion = "0"
		elif int(emotion) in [3,5]:
			emotion = "1"
		elif int(emotion) in [6]:
			emotion = "2"
		emotion = keras.utils.to_categorical(emotion, num_classes_alternative)
...
# model.add(keras.layers.Dense(num_classes_alternative, activation="softmax"))
...
# model.save("face_model_3classes.h5")
# del model
